...
from stable_baselines3.common.buffers import RolloutBuffer, RecurrentDictRolloutBuffer, RecurrentRolloutBuffer
...
class RecurrentPPO(OnPolicyAlgorithm):
    ...
    def _setup_model(self) -> None:
        ...
        if not isinstance(self.policy, LSTMPolicy):
    ...
    def collect_rollouts(
        ...
        while n_steps < n_rollout_steps:
            ...
            if name == "CartPole-v1" and reward_type == "STL":
                ...
                if callback.ctr == 0:
                    if sem == "cau":
                        ...
                        stl_buf = 'alw_[0,1500](speed[t]<0.1 and angle[t]<0.02 and loc[t]<0.5)'
                        stl_buf = 'alw_[0,1500](angle[t]<0.2 and loc[t]<0.5)'
                    # try:
                    #     spec.parse()
                    #     spec.pastify()
                ...
                if sem == "cau":
                    ...
                    if dones[0] and callback.ctr>=500:
                        rob += 200
            ...
            if name == "PointWallGoal1-v0" and reward_type == "STL":
                #问题：定义一种新的object，设置该object的雷达监测
                dg = 0.
                dc = 0.
                dl = 0.

                dg = max( new_obs[0][3:19])
                dl=0.
                dc = max( new_obs[0][28:44])

                if dg>0:
                    Dg = -np.log(dg)
                else:
                    Dg = env.envs[0].lidar_max_dist
                if dc>0:
                    Dc = -np.log(dc)
                else:
                    Dc = env.envs[0].lidar_max_dist
                if dl>0:
                    Dl = -np.log(dl)
                else:
                    Dl = env.envs[0].lidar_max_dist
                u = clipped_actions
                if callback.ctr == 0:
                    if sem == "cau":
                        signal_buf = 'dg,dc,dl'
                        stl_buf = 'alw_[0,1500](ev_[0,40](dg[t]<=0.2) and dc[t]>0.15)'
                    ...
                    #     sys.exit()
                if sem == "cau":
                    data = np.array([n_steps, Dg, Dc, Dl])
                    data_in.append(data)
                    # print(data_in)
                    if cau == "cau":
                        robs = stl_causation_opt.mainFunction(signal_buf, stl_buf, np.array(data_in), np.array([0]))
