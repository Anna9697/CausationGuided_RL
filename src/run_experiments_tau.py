...
def run_exp(env_id, reward_id, sem, run, tau):
    print(int(run))
    ...
    if env_id=="Hopper-v3":
        ...
        if sem == 'cau' or sem == 'cau_app':
            ...
            env = GymSeq(env_pre.envs[0], n_steps=tau)
        else:
            env = GymSeq(env_pre.envs[0], n_steps=1)

        log_path = "./log_tau_" + str(tau) + "/sac_hopper_tensorboard"
        model = SAC('MlpPolicy', env, learning_starts=3000, use_sde=False,  tensorboard_log=log_path, verbose=1, seed=int(run)%10)
        model.learn(total_timesteps=int(2e6), max_episodes=1600, reward_type=reward_type, stl=name, sem=sem, run=run, data_len=tau)
        ...
        log_path = "./log_tau_" + str(tau) + "/ppo_cartpole_tensorboard"
        model = PPO('MlpPolicy', env, tensorboard_log=log_path, learning_rate=4e-4, verbose=1, seed=int(run)%10+30)
        model.learn(total_timesteps=int(1e6), max_episodes=500, reward_type=reward_type, stl=name, sem=sem, run=run, data_len=tau)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--tau", help="tau", type=int, default=16)
    ...
    for i in range(10):
        run_exp(env_id, reward, sem, str(int(run)+i), tau)
